...
Основной класс, отвечающий за подписку на события и их активацию в приложении. Является брокером событий. Имеет одно поле - **_events**. Это коллекция, в которую записываются названия и ссылки на функции, которые будут вызываться при событиях. Методы:
